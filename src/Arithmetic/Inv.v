...
(*Evaluation function for multi-limbg in twos complement *)
...
Proof.
  ...
  - destruct f.
    ...
    + unfold sat_arithmetic_shiftr1; simpl.
      ...
      unfold Z.arithmetic_shiftr1; rewrite !Z.land_pow2_testbit, !eval_testbit; try tauto; try nia; simpl.
...
Proof.
  pose proof Z.twos_complement_bounds a m Hm. apply Z.twos_complement_spec; try split; try lia.
...
Proof. apply Z.twos_complement_spec; [lia|split]; [|pose proof Z.twos_complement_bounds a m Hm; lia].
       rewrite <- twos_complement_zopp, twos_complement_opp_correct, Z.twos_complement_mod', Z.mod_mod; try lia.
       apply Z.pow_nonzero; lia. Qed.
...
Proof.
  replace (0 <? Z.twos_complement m a) with (- Z.twos_complement m a <? 0) by
      (destruct (0 <? Z.twos_complement m a) eqn:?; destruct (- Z.twos_complement m a <? 0) eqn:?;
                rewrite ?Z.ltb_lt in *; rewrite ?Z.ltb_ge in *; try reflexivity; try lia).
...
Proof.
  assert (0 < Z.of_nat n) by omega. unfold eval_twos_complement. rewrite eval_sat_arithmetic_shiftr1, Z.arithmetic_shiftr1_spec; auto; try nia.
...
Proof. assert (0 < Z.of_nat n) by omega; unfold eval_twos_complement in *; rewrite eval_sat_opp, Z.twos_complement_mod, twos_complement_zopp; try nia; tauto. Qed.
...
Proof.
  ...
  rewrite twos_complement_pos_spec, <- (eval_twos_complement_sat_mod2 machine_wordsize sat_limbs), Zmod_odd; auto; try nia.
  ...
  - apply andb_false_iff in E.
    destruct E; rewrite H;
      unfold Z.zselect; cbv [fst]; simpl (if _ =? _ then _ else _);
        ...
        rewrite Z.twos_complement_mod, Z.twos_complement_add_full, Z.twos_complement_one; try rewrite ?Z.twos_complement_one; lia. Qed.
...
Proof.
  ...
  rewrite sat_opp_mod2; auto.
  rewrite twos_complement_pos_spec, <- !(eval_twos_complement_sat_mod2 machine_wordsize sat_limbs); try lia.
